...
export function AuthCard({ title, description, eyebrow, children, footer, wide = false, className }: AuthCardProps) {
  return (
    <section
      className={cn(
        'pillar-top-blue w-full overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-lift',
        wide ? 'max-w-2xl' : 'max-w-md',
        className,
      )}
    >
      <div className="px-6 pb-6 pt-8 text-center sm:px-10">
        <div className="mx-auto mb-5 flex h-20 w-20 items-center justify-center rounded-full bg-blue-50 ring-8 ring-blue-50/60">
          <Emblem size={56} decorative />
        </div>
        {eyebrow ? <Ribbon tone="blue">{eyebrow}</Ribbon> : null}
