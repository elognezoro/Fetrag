...
export interface UserMenuLink {
  ...
  icon?: UserMenuIcon
  external?: boolean
  /** Sépare visuellement les liens personnels, les liens de rôle et les liens vers l'autre application. */
  group?: UserMenuGroup
}
...
/** Menu utilisateur (avatar + liens selon le rôle + déconnexion). */
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" sideOffset={8} className="w-72">
        <div className="flex flex-col gap-0.5 px-3 py-2">
          ...
          <span className="mt-1.5 inline-flex w-fit rounded-full bg-blue-50 px-2 py-0.5 text-[11px] font-bold uppercase tracking-wider text-blue-700">
            {user.roleLabel}
          </span>
        </div>
        {groups.map((group) => (
