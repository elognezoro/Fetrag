  ...
  try {
    ...
    if (typeof processJobs !== 'function') {
      return NextResponse.json(
        { ok: true, skipped: true, reason: 'Le processeur de jobs n’est pas encore disponible.', app: 'web' },
        { status: 200, headers: { 'Cache-Control': 'no-store' } },
      )
    }
    const result = await (processJobs as ProcessJobs)({ limit: 25, workerId: `cron-web-${process.env.VERCEL_REGION ?? 'local'}` })
    logger.info('cron.jobs', { app: 'web', ...result, durationMs: Date.now() - startedAt })
    return NextResponse.json(
      { ok: true, app: 'web', ...result, durationMs: Date.now() - startedAt, at: new Date().toISOString() },
      { headers: { 'Cache-Control': 'no-store' } },
    )
  } catch (error) {
    logger.error('cron.jobs.failed', { app: 'web', error: error instanceof Error ? error.message : String(error) })
