...
export default async function DeconnexionPage() {
  ...
  if (!session?.user?.id) {
    return (
      <AuthCard eyebrow="Session" title="Vous êtes déconnecté" description="Votre session est fermée. À bientôt sur fetrag.ga.">
  ...
  return (
    <AuthCard
      ...
      title={
        <>
          Se <span className="italic text-blue-600">déconnecter</span> ?
        </>
      }
      description={
        <>
          Vous êtes connecté en tant que <span className="font-semibold text-navy">{session.user.email ?? session.user.name}</span>.
        </>
      }
    >
      <form action={logoutAction} className="space-y-3">
        ...
        <Button asChild variant="ghost" size="md" className="w-full">
          <Link href="/espace">Annuler et rester connecté</Link>
        </Button>
      </form>
    </AuthCard>
  )
